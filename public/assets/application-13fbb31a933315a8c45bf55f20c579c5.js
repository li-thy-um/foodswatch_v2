...
(function() {
  var Micropost, User, render_microposts,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };
  ...
  User = (function() {
    ...
    User.prototype.get = function(option) {
      return $.get("/api/v1/users/" + this.id + "/" + option.action + "?page=" + option.page + "&query=" + option.query, (function(_this) {
        return function(res) {
  ...
  Micropost = (function() {
    function Micropost(micropost, page) {
      this.original_post = bind(this.original_post, this);
      this.foods_panel = bind(this.foods_panel, this);
      this.action_panel = bind(this.action_panel, this);
      this.btn_delete = bind(this.btn_delete, this);
      this.btn_like = bind(this.btn_like, this);
      this.btn_share = bind(this.btn_share, this);
      this.btn_comment = bind(this.btn_comment, this);
      var dom_id;
      this.page = page;
      this.micropsot = micropost;
      dom_id = this.page === "share" ? "share_" + micropost.id : micropost.id;
      this.dom = "<div class='panel panel-default micropost' id='" + dom_id + "'> <div class='panel-body'> <div class='row-fluid'> <div class='media'> <div class='media-left'> " + micropost.user.avatar + " </div> <div class='media-body'> <span class='media-heading media-body'> " + micropost.user.link + " </span> <br> <span class='content'> " + micropost.content + " <span> " + (this.foods_panel(micropost.foods)) + " " + (this.original_post(micropost.original_post)) + " <div class='timestamp'> <span> " + micropost.timestamp + " </span> </div> </div> </div> </div> </div> " + (this.action_panel(micropost)) + " </div>";
      this.$dom = $(this.dom).hide();
    }

    Micropost.prototype.btn_comment = function(micropost) {
      return "<a class='btn btn-comment btn-action btn-primary'> <span> 评论 " + micropost.count.comment + " </span> </a>";
    };

    Micropost.prototype.btn_share = function(micropost) {
      return "<a class='btn btn-share btn-action btn-success'> <span> 分享 " + micropost.count.share + " </span> </a>";
    };

    Micropost.prototype.btn_like = function(micropost) {
      if (micropost.is_liking) {
        return "<a class='btn btn-like btn-liked btn-default' data-remote='true' rel='nofollow' data-method='delete' href='/likes/" + micropost.like_id + "'> <i class='fa fa-thumbs-o-up'> " + micropost.count.like + " </i> </a>";
      } else {
        return "<a class='btn btn-like btn-default' data-remote='true' rel='nofollow' data-method='post' href='/likes?like[micropost_id]=" + micropost.id + "'> <i class='fa fa-thumbs-o-up'> " + micropost.count.like + " </i> </a>";
      }
    };

    Micropost.prototype.btn_delete = function(micropost) {
      if ($('body').data('user') !== micropost.user.id) {
        return "";
      }
      return "<a class='btn btn-action btn-danger btn-delete' data-confirm='确定删除?' data-remote='true' rel='nofollow' data-method='delete' href='/microposts/" + micropost.id + "'> 删除 </a>";
    };

    Micropost.prototype.action_panel = function(micropost) {
      if (this.page === 'share') {
        return "";
      }
      return "<div class='panel-footer'> <div class='post-action'> <div class='post_action_panel_" + micropost.id + "'> " + (this.btn_delete(micropost)) + " " + (this.btn_share(micropost)) + " " + (this.btn_comment(micropost)) + " " + (this.btn_like(micropost)) + " </div> </div> </div>";
    };

    Micropost.prototype.foods_panel = function(foods) {
      var html;
      if (foods.length === 0) {
        return "";
      }
      ...
      foods.forEach(function(food) {
        return html += "<a class='food-tag' data-id='" + food.id + "'> <i class='fa fa-cutlery'></i> " + food.name + " </a>";
      });
      html += "</div>";
      return html;
    };

    Micropost.prototype.original_post = function(post) {
      if (!post) {
        return "";
      }
      return "<div class='panel-body'> " + (new Micropost(post, 'share').dom) + " </div>";
    };
  ...
  $(document).on("ready page:load", function() {
    ...
    if (($dom = $("[data-action='microposts']")).length > 0) {
      return render_microposts($dom);
    }
  });

  render_microposts = function($dom, action) {
    var page;
    page = $dom.data('page') || 1;
    return new User($dom.data('user')).get({
      action: action || "microposts",
      query: action === "search" ? $dom.data("query") : "",
      page: page,
      success: function(res) {
        $dom.empty();
        return res.data.forEach(function(data) {
          var micropost;
          micropost = new Micropost(data);
          return $dom.append(micropost.$dom.fadeIn());
        });
      }
    });
  };
...
(function() {
  ...
  $(document).on('ready page:load', function() {
    if ($('#chart_test').length) {
      return new CalorieChart('chart_test').render();
    }
  });

  CalorieChart = (function() {
    function CalorieChart(target_id) {
      this.data_points = bind(this.data_points, this);
      this.transform_data = bind(this.transform_data, this);
    ...
    CalorieChart.prototype.render = function() {
      return $.get('calorie.json', (function(_this) {
        return function(data) {
          _this.data = data;
          _this.option.data = _this.transform_data();
          return new CanvasJS.Chart(_this.target_id, _this.option).render();
        };
      })(this));
    };

    CalorieChart.prototype.transform_data = function() {
      return [
        {
          name: "碳水热量",
          type: 'carb',
        ...
        return function(t) {
          return {
            ...
            name: t.name,
            color: t.color,
            dataPoints: _this.data_points(t.type)
          };
        };
      })(this));
    };

    CalorieChart.prototype.data_points = function(type) {
      return this.data['labels'].map((function(_this) {
        return function(lab, i) {
          return {
            label: lab,
            y: _this.data[type][i]
          };
...
(function() {
  $(function() {
    return $(document).on('click', '#forget_password', function() {
      var $form, original_action;
      $form = $('#sign_in_form');
      original_action = $form.attr('action');
      $form.attr('action', '/passwords').submit();
      return $form.attr('action', original_action);
    });
  });

}).call(this);
(function() {
  var MicropostCollapse,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };

  $(function() {
    var init_collapse;
    init_collapse = function($micropost) {
      var collapse, micropost_id;
      micropost_id = $micropost.attr('id');
      collapse = new MicropostCollapse(micropost_id);
      collapse.render($micropost.find('.panel-footer'));
      $micropost.find('.btn-share').bind('collapse', function() {
        return collapse.act('share', $(this));
      });
      return $micropost.find('.btn-comment').bind('collapse', function() {
        return collapse.act('comment', $(this));
      });
    };
    return $(document).on('click', '.micropost .btn-share, .micropost .btn-comment', function() {
      var $micropost;
      $micropost = $(this).closest('.micropost');
      if ($micropost.find('.micropost-collapse').length === 0) {
  ...
  MicropostCollapse = (function() {
    function MicropostCollapse(micropost_id) {
      this._content_for = bind(this._content_for, this);
      this._hide = bind(this._hide, this);
      this._replace = bind(this._replace, this);
      this._show = bind(this._show, this);
      this._error = bind(this._error, this);
      this.act = bind(this.act, this);
      this.render = bind(this.render, this);
      this._action = '';
      this._micropost_id = micropost_id;
      this._dom = $("<div class='micropost-collapse'></div>").hide();
    }

    MicropostCollapse.prototype.render = function($target) {
      return $target.append(this._dom);
    };

    MicropostCollapse.prototype.act = function(action, $btn) {
      if (this._action === '') {
        $btn.addClass('disabled');
        this._content_for(action, (function(_this) {
          return function(content) {
            ...
            if (content === false) {
              ...
              return _this._show(content);
            }
          };
        })(this));
      } else if (this._action !== action) {
        $btn.addClass('disabled');
        this._content_for(action, (function(_this) {
          return function(content) {
            $btn.removeClass('disabled');
    ...
    MicropostCollapse.prototype._error = function() {
      return alert('出错啦，很有可能是这个微博刚刚被删除了，刷新一下看看吧！');
    };

    MicropostCollapse.prototype._show = function(content) {
      this._dom.append(content);
      return this._dom.slideDown();
    };

    MicropostCollapse.prototype._replace = function(content) {
      this._dom.empty();
      return this._dom.append(content);
    };

    MicropostCollapse.prototype._hide = function() {
      return this._dom.slideUp((function(_this) {
        return function() {
          return _this._dom.empty();
        };
      })(this));
    };

    MicropostCollapse.prototype._content_for = function(action, render) {
      return $.get("/collapses/" + action + "/" + this._micropost_id, (function(_this) {
        return function(content) {
...
(function() {
  ...
  refresh_notice = function() {
    if ($('#notice').length > 0) {
      return $.get('/notices/count.js');
    }
  };
...
(function() {
  $(function() {
    $(document).on('click', 'div.search .dropdown-menu>li>a', function() {
      var $select, text, type;
      $select = $(this);
      text = $select.html();
      type = $select.data('type');
      $('div.search .dropdown-toggle>span:first').html(text);
      return $('div.search form').attr('action', "/search/" + type);
    });
    return $(document).on('click', 'div.search .btn-search', function() {
      return $('div.search form').submit();
    });
  });
...
(function() {
  $(function() {
    ...
    return $(document).on('change', '#user_avatar', function() {
      return $('#user_avatar_form').submit();
    });
  });
